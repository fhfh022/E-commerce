        ...
        <div className="mx-6">
          <div className="flex items-center justify-between max-w-7xl mx-auto py-4 transition-all">
            ...
            >
              ...
              <Link href="/shop" className="transition hover:text-green-600">Shop</Link>
              <Link href="/" className="transition hover:text-green-600">About</Link>
              <Link href="/" className="transition hover:text-green-600">Contact</Link>
              ...
              <div className="relative hidden xl:block">
                ...
                {isSearchOpen && search.length >= 2 && searchSuggestions.length > 0 && (
                    ...
                    <ul className="py-1">
                      {searchSuggestions.map((product) => (
                        ...
                        >
                            ...
                            <span className="text-xs text-green-600 font-semibold">${product.price}</span>
                        </li>
                      ))}
                    </ul>
              ...
              {
                !user ? (
                  <button onClick={openSignIn} className="px-8 py-2 bg-indigo-500 rounded-full text-white transition hover:bg-indigo-600">
                    Login
                  </button>
       ...
       {/* --- MOBILE MENU DROPDOWN --- */}
        ...
        >
          <div className="flex flex-col gap-4 px-6 py-6 border-t border-gray-100 text-slate-600 font-medium">
            <Link href="/" onClick={() => setIsMenuOpen(false)} className="transition hover:text-green-600">Home</Link>
            <Link href="/shop" onClick={() => setIsMenuOpen(false)} className="transition hover:text-green-600">Shop</Link>
            <Link href="/" onClick={() => setIsMenuOpen(false)} className="transition hover:text-green-600">About</Link>
            <Link href="/" onClick={() => setIsMenuOpen(false)} className="transition hover:text-green-600">Contact</Link>
            ...
            {/* ====== ส่วน Login/User ที่แก้ไขให้แสดงผลถูกต้อง ====== */}
            {
              !user ? (
                ...
                <button onClick={openSignIn} className="w-full py-3 bg-indigo-500 rounded-full text-white text-center mt-2 transition hover:bg-indigo-600">
                  Login
                </button>
              ) : (
                ...
                <div className="mt-2 pt-2 border-t border-slate-100">
                    <div className="flex items-center justify-between text-sm text-slate-500 mb-2">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-semibold text-slate-700">{user.username || user.fullName}</span>
                        </div>
