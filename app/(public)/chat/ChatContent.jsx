...
export default function ChatPage() {
  ...
  // ✅ 1. เพิ่ม State สำหรับเก็บ Role ที่ดึงจาก DB โดยตรง
  const [dbRole, setDbRole] = useState(null);

  // ดึงจาก Redux (เผื่อมี)
  const reduxRole = useSelector((state) => state.user?.role);

  // Logic เช็ค Admin: ถ้ามีใน Redux ใช้ Redux, ถ้าไม่มีใช้จาก DB
  const currentRole = reduxRole || dbRole;
  const isAdmin = currentRole === "master_admin" || currentRole === "admin";
  ...
  // Ref เพื่อจำห้องที่เลือกล่าสุด (แก้ปัญหาข้อความหาย)
  const selectedRoomIdRef = useRef(selectedRoomId);

  // ซิงค์ค่า Ref กับ State ทุกครั้งที่เปลี่ยนห้อง
  useEffect(() => {
        ...
        setMessages([
          {
            id: "welcome-msg",
            is_admin: true,
            content: `สวัสดีครับคุณ ${
              user?.firstName || "ลูกค้า"
            }! 👋 มีสินค้าชิ้นไหนให้ PRT Assistant ช่วยแนะนำ หรือพบปัญหาการใช้งานสอบถามได้เลยนะครับ`,
  ...
  useEffect(() => {
    ...
    const fetchInitData = async () => {
      // โหลดครั้งแรกให้หมุน Loading แต่ถ้าเป็นการ update จาก Realtime ไม่ต้องหมุน
      if (chatRooms.length === 0) setLoading(true);

      // ✅ 2. ดึง 'role' จาก Database มาด้วย เพื่อความชัวร์
      ...
      const isAdminLocal =
        dbUser.role === "master_admin" || dbUser.role === "admin";

      if (isAdminLocal) {
        // ✅ 3. ดึง role ของคู่สนทนามาด้วย (เพิ่ม role ใน select)
        const { data: rooms } = await supabase
          .from("chat_rooms")
          .select(`*, user:users(name, email, avatar, role)`)
          .order("updated_at", { ascending: false });

        // ✅ 4. กรอง Admin/Master Admin ออกจากรายการ Inbox
        // (แสดงเฉพาะคนที่ role ไม่ใช่ admin และ master_admin)
        const customerRooms = (rooms || []).filter(
          (room) =>
            room.user_id !== dbUser.id &&
            room.user?.role !== "admin" &&
            room.user?.role !== "master_admin"
        );

        setChatRooms(customerRooms);

        // Logic เลือกห้องอัตโนมัติ: เช็คจาก Ref ว่า "ยังไม่เลือกห้อง" ใช่ไหม ถึงค่อยเลือก
        if (
  ...
  useEffect(() => {
    ...
    // Subscribe Realtime ข้อความในห้อง
      ...
      <div className="flex w-full max-w-6xl h-full bg-white sm:rounded-2xl shadow-2xl overflow-hidden border border-slate-200 relative transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {chatRooms.map((room) => (
                <div
                  ...
                  className={`p-3 rounded-xl cursor-pointer transition-all flex gap-3 items-center border border-transparent ${
                ...
                >
                  <div className="size-11 rounded-full bg-slate-200 flex-shrink-0 overflow-hidden relative shadow-inner">
                    {room.user?.avatar ? (
                      ...
                      <User
                        size={18}
                        className="m-auto mt-2.5 text-slate-500"
                      />
                    )}
        ...
        <div
          className={`flex-1 flex flex-col bg-white relative ${
            isAdmin && !selectedRoomId ? "hidden md:flex" : "flex"
          }`}
        >
          {selectedRoomId ? (
            <>
                        ...
                        <div
                          className={`p-3 rounded-2xl text-sm leading-relaxed shadow-sm break-words ${
                            isMe
                              ...
                              : "bg-white text-slate-800 border border-slate-200 rounded-2xl rounded-tl-none font-medium"
                          }`}
                        >
                          {/* แสดงรูปภาพ */}
                          {msg.image_url && (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 animate-in fade-in duration-500">
              <div className="size-20 bg-indigo-50 rounded-full flex items-center justify-center mb-5 text-indigo-300 shadow-inner border border-indigo-100">
