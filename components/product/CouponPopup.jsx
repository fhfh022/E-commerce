        ...
        const fetchCoupons = async () => {
            try {
                ...
                const { data } = await supabase
                    .from('coupons')
                    .select('code, discount_value, discount_type, description, expiry_date, quantity, used_count')
                    .eq('is_active', true)
                ...
                if (data && data.length > 0) {
                    ...
                    const availableCoupons = data.filter(c => c.used_count < c.quantity);
            ...
            >
                <div className="flex items-center justify-between p-5 bg-slate-50 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                        ...
                        <div><h3 className="font-bold text-slate-900">Available Coupons</h3></div>
                    </div>
                    <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 transition"><ChevronRight size={20} /></button>
                </div>
                    ...
                    {coupons.map((coupon, index) => (
                        <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group overflow-hidden">
                            <div className="relative z-10">
                                ...
                                {/* แสดงจำนวนคงเหลือ */}
                                <p className="text-xs text-slate-400 mt-1">Left: {coupon.quantity - coupon.used_count} / {coupon.quantity}</p>
                                ...
                                <div className="mt-4 flex items-center gap-2">
                                    ...
                                    <button onClick={() => handleCopy(coupon.code)} className="bg-slate-900 text-white p-2 rounded-lg hover:bg-blue-600 transition">
                                        <Copy size={16} />
                                    </button>
                                </div>
                            </div>
                ...
                <div className="absolute bottom-0 w-full p-4 bg-white border-t border-slate-100">
                    <button onClick={() => setIsVisible(false)} className="text-xs text-slate-400 hover:text-red-500 w-full flex items-center justify-center gap-1 py-1">
