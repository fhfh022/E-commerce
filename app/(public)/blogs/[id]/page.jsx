...
import { Calendar, User, Eye, ChevronLeft, Share2, Clock } from "lucide-react";
...
export default async function BlogDetail({ params }) {
  ...
  return (
    <div className="bg-white min-h-screen pb-10 sm:pb-20">
      ...
      <div className="relative h-[300px] sm:h-[400px] lg:h-[500px] w-full bg-slate-900">
        {blog.image_url ? (
            <Image 
                src={blog.image_url} 
                alt={blog.title} 
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 sm:p-10 md:p-16 text-white max-w-7xl mx-auto inset-x-0">
            <Link 
                href="/blogs" 
            ...
            <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-black leading-tight mb-2 sm:mb-4 drop-shadow-xl text-balance max-w-5xl">
                {blog.title}
            </h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 sm:-mt-16 relative z-10">
        <div className="bg-white p-6 sm:p-10 md:p-16 rounded-3xl sm:rounded-[2.5rem] shadow-2xl border border-slate-100">
            ...
            {blog.excerpt && (
                <div className="mb-8 sm:mb-12 pl-4 sm:pl-6 border-l-4 border-indigo-500">
                    <p className="text-md sm:text-lg md:text-2xl font-medium text-slate-700 leading-relaxed font-serif italic">
                        "{blog.excerpt}"
                    </p>
                </div>
            )}

            <hr className="border-slate-100 mb-8 sm:mb-12" />

            {/* Main Content */}
            <article 
                className="prose prose-base sm:prose-lg md:prose-xl prose-slate max-w-none
                prose-headings:font-bold prose-headings:text-slate-900 prose-headings:mt-8 sm:prose-headings:mt-12 prose-headings:mb-4 sm:prose-headings:mb-6
                prose-p:text-slate-600 prose-p:leading-loose prose-p:mb-6 sm:prose-p:mb-8
                prose-a:text-indigo-600 prose-a:no-underline hover:prose-a:underline
                prose-img:rounded-2xl sm:prose-img:rounded-3xl prose-img:shadow-lg prose-img:my-8 sm:prose-img:my-10 prose-img:w-full
            ...
            <div className="mt-12 sm:mt-20 pt-8 sm:pt-10 border-t border-slate-100 flex flex-col items-center">
                ...
                <button className="flex items-center gap-2 px-6 sm:px-8 py-2.5 sm:py-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-full font-bold text-slate-600 transition active:scale-95 shadow-sm hover:shadow-md cursor-not-allowed opacity-70 text-sm sm:text-base">
                    <Share2 size={18} /> คัดลอกลิงก์
                </button>
            </div>
